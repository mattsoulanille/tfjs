...
describe('ESM Module Provider', () => {
  it('importCoreStr forwardModeOnly=true', () => {
    ...
    expect(res).toContain(`export * from '@tensorflow/tfjs-core/src/base';`);
  });
  ...
  it('importBackendStr cpu', () => {
    ...
    expect(res).toBe(`export * from '@tensorflow/tfjs-backend-cpu/src/base';`);
  });

  it('importBackendStr webgl', () => {
    const res = esmImportProvider.importBackendStr('webgl');
    expect(res).toBe(
        `export * from '@tensorflow/tfjs-backend-webgl/src/base';`);
  });
  ...
  it('importKernelStr Max cpu', () => {
    ...
    expect(res.importStatement)
        .toContain(`from '@tensorflow/tfjs-backend-cpu/src/kernels/Max'`);
  ...
  it('importGradientConfigStr Max', () => {
    ...
    expect(res.importStatement)
        .toContain(`from '@tensorflow/tfjs-core/src/gradients/Max_grad'`);
  ...
  it('importOpForConverterStr add', () => {
    ...
    expect(res).toBe(`export {add} from '@tensorflow/tfjs-core/src/ops/add';`);
  });

  it('importOpForConverterStr gatherND', () => {
    const res = esmImportProvider.importOpForConverterStr('gatherND');
    expect(res).toBe(
        `export {gatherND} from '@tensorflow/tfjs-core/src/ops/gather_nd';`);
  });
  ...
  it('importOpForConverterStr batchToSpaceND', () => {
    ...
    expect(res).toBe(
        ...
        `export {batchToSpaceND} from '@tensorflow/tfjs-core/src/ops/batch_to_space_nd';`);
  });

  it('importOpForConverterStr concat1d', () => {
    const res = esmImportProvider.importOpForConverterStr('concat1d');
    expect(res).toBe(
        `export {concat1d} from '@tensorflow/tfjs-core/src/ops/concat_1d';`);
  });
  ...
  it('importOpForConverterStr avgPool3d', () => {
    ...
    expect(res).toBe(
        // tslint:disable-next-line:max-line-length
        `export {avgPool3d} from '@tensorflow/tfjs-core/src/ops/avg_pool_3d';`);
  });
  ...
  it('importOpForConverterStr stridedSlice', () => {
    ...
    expect(res).toBe(
        // tslint:disable-next-line: max-line-length
        `export {stridedSlice} from '@tensorflow/tfjs-core/src/ops/strided_slice';`);
  });
  ...
  it('importNamespacedOpsForConverterStr image.resizeBilinear', () => {
    ...
    expect(res).toBe(
        // tslint:disable-next-line: max-line-length
        `import {resizeBilinear as resizeBilinear_image} from '@tensorflow/tfjs-core/src/ops/image/resize_bilinear';
export const image = {
\tresizeBilinear: resizeBilinear_image,
  ...
  it('importNamespacedOpsForConverterStr two ops in namespace', () => {
    ...
    expect(res).toBe(
        // tslint:disable-next-line: max-line-length
        `import {resizeBilinear as resizeBilinear_image} from '@tensorflow/tfjs-core/src/ops/image/resize_bilinear';
import {resizeNearestNeighbor as resizeNearestNeighbor_image} from '@tensorflow/tfjs-core/src/ops/image/resize_nearest_neighbor';
export const image = {
\tresizeBilinear: resizeBilinear_image,
